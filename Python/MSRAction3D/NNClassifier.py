...
def knn(i,task_per_cpu,k_num,k_pool,dataset,options,Macro,Micro,rightnum):
    ...
    for j in range(start,end):
        ...
        for k_count in range(k_num):
            ...
            if predict==dataset.testsetlabel[j]:
                rightnum[k_count] += 1

        temp_dis = -dis_totrain_scores
        temp_dis[np.nonzero(np.isnan(temp_dis))] = 0
        # logger.info("{:.1f}%".format(j/testsetdatanum*100))
        Macro[j] = metrics.average_precision_score(dataset.trainsetlabelfull[:,dataset.testsetlabel[j]-1],temp_dis, 'macro')
        Micro[j] = metrics.average_precision_score(dataset.trainsetlabelfull[:,dataset.testsetlabel[j]-1],temp_dis, 'micro')
...
def NNClassifier(dataset,options,method='dtw'):
    ...
    inner_process = [Process(target=knn, args=(i,task_per_cpu,k_num,k_pool,dataset,options,Macro,Micro,rightnum,method)) for i in range(cpu_count-1)]

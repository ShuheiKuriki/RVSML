...
def loadFile(fpath,embeddings,word2id):
    with io.open(fpath, 'r', encoding='utf-8') as f:
        ...
        for line in f:
            ...
            for s in space_split:
                ...
                if s[-1] not in ',.':
                    if s in word2id:
                        emb = embeddings[word2id[s]]
                    else:
                        emb = np.zeros(300)
                    words.append(emb)
                else:
                    if s[:-1] in word2id:
                        emb = embeddings[word2id[s[:-1]]]
                    else:
                        emb = np.zeros(300)
                    words.append(emb)
                    words.append(embeddings[word2id[s[-1]]])   
    words_np = np.zeros((len(words),300))
    for i,w in enumerate(words):
        words_np[i] = w
    return words_np

def read_txt_embeddings(params, source, full_vocab=True):
    """
